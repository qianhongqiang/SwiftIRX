...
awk '
  ...
  function process_block(    actor_class, context, current, kind,
                              protocol_key, root, root_index) {
    if (symbol == "")
      return
    ...
    for (current = 1; current <= line_count; ++current) {
      kind = node_kind(lines[current])
      if (kind == "Class" && actor_class == 0)
        actor_class = current
      if (kind == "Protocol") {
        protocol_key = subtree_key(current)
        if (protocol_key ~ /kind=Module, text="Swift"/ &&
            protocol_key ~ /kind=Identifier, text="Actor"/ &&
            actor_class != 0)
